...
//            self.loadPage(index: index)
    ...
    public lazy var pageScrollView:AMCollectionView = {
        let layout = UICollectionViewFlowLayout()
        layout.minimumLineSpacing = 0
        layout.minimumInteritemSpacing = 0
        
        layout.scrollDirection = .horizontal
        let pageScroll = AMCollectionView(frame: .zero, collectionViewLayout: layout)
...
public class AMCollectionView:UICollectionView,UIGestureRecognizerDelegate{
...
//        if([gestureRecognizer locationInView:self.window].x < 30 && [otherGestureRecognizer isKindOfClass:UIScreenEdgePanGestureRecognizer.class]){
//             return true;
//        }else{
//            return false;
//        }
...
//    }
    public func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldRequireFailureOf otherGestureRecognizer: UIGestureRecognizer) -> Bool {
        if gestureRecognizer .location(in: self.window).x < 30 && otherGestureRecognizer .isKind(of: UIScreenEdgePanGestureRecognizer.self){
            return true
        }
        return false
    }
