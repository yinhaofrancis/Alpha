...
public class CokeMetalConfiguration{
    ...
    public func createCIImage(texture:MTLTexture)->CIImage?{
        let op = [
            CIImageOption.colorSpace:CGColorSpaceCreateDeviceRGB(),
            CIContextOption.outputPremultiplied:true,
            CIContextOption.useSoftwareRenderer: false,
        ] as? [CIImageOption : Any]
        return CIImage(mtlTexture: texture, options: op)
    }
...
public class CokeRenderConfigration{
    public var vertex:String
    public var fragment:String
    unowned var config:CokeMetalConfiguration
    public init(vertex:String,fragment:String,config:CokeMetalConfiguration){
        self.vertex = vertex
    ...
    public var renderPipelineDescriptor:MTLRenderPipelineDescriptor{
        ...
        p.vertexFunction = self.config.function(name: self.vertex)
        p.fragmentFunction = self.config.function(name: self.fragment)
        p.colorAttachments[0].pixelFormat = CokeConfig.metalColorFormat
        return p
    }
