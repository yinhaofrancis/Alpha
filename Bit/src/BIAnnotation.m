...
NSArray<NSString*>* BIReadSlot(char* sectionName,const struct mach_header* mhd){
...
static void dyld_callback(const struct mach_header *mhp, intptr_t vmaddr_slide){
    NSArray<NSString*>* mods = BIReadSlot(BISectModuleName, mhp);
        ...
        if(!e){
            if([json isKindOfClass:NSDictionary.class] && [json allKeys].count){
                ...
                if(name.length > 0 && clsName.length > 0){
                    [BIModuleManager.shared regModuleWithName:name implement:NSClassFromString(clsName)];
                }
            }
        }
    }
    NSArray<NSString*>* cu = BIReadSlot(BISectCustom, mhp);
